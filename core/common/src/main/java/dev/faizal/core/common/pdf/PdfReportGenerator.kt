...
class PdfReportGenerator(private val context: Context) {
     ...
     * Generate PDF report.
     *
     * @param storeName nama toko untuk header — fallback "ZYPOS" kalau null
     * @param storeAddress alamat untuk subtitle — opsional
     */
    ...
    fun generateReport(
        ...
        val displayName = storeName?.takeIf { it.isNotBlank() } ?: "ZYPOS"

        val p1 = document.startPage(PdfDocument.PageInfo.Builder(pageWidth, pageHeight, 1).create())
        var y = margin
        y = drawCoverHeader(p1.canvas, month, year, displayName, storeAddress)
        y += 14f
    ...
    private fun drawCoverHeader(
        ...
        // STORE NAME (uppercase, jadi judul utama)
        canvas.drawText(
            storeName.uppercase(),
            margin + 52f, 46f,
            boldPaint(22f, colorWhite),
        )

        // Subtitle: address kalau ada, fallback ke "MONTHLY SALES REPORT"
        val subtitle = storeAddress?.takeIf { it.isNotBlank() }
            ?: "MONTHLY SALES REPORT"
        canvas.drawText(
            subtitle, margin + 52f, 62f,
        ...
        canvas.drawRoundRect(
            margin, 72f, margin + pillW, 92f, 10f, 10f,
    ...
    private fun drawPageHeader(
        canvas: Canvas, startY: Float, title: String, subtitle: String, storeName: String,
    ): Float {
        var y = startY

        canvas.drawRoundRect(
            margin, y, margin + 4f, y + 38f, 2f, 2f,
            Paint().apply { color = colorAccent; style = Paint.Style.FILL },
        )
        canvas.drawRect(
            margin + 2f, y, margin + 4f, y + 38f,
            Paint().apply { color = colorAccent; style = Paint.Style.FILL },
        )

        canvas.drawText("${storeName.uppercase()} Report", margin + 14f, y + 13f, paintCaption)
    ...
    private fun drawSummaryCards(canvas: Canvas, startY: Float, sales: MonthlySalesReport?): Float {
        ...
        val cards = listOf(
            ...
            Triple(
                "Avg Order Value",
                numberFormat.format(
                    if (sales.orderCount > 0) sales.totalAmount / sales.orderCount else 0.0,
                ),
                2,
            ),
        )
    ...
    private fun drawGrowthIndicator(canvas: Canvas, startY: Float, growth: SalesGrowth): Float {
        ...
        canvas.drawRoundRect(
            margin, y, margin + contentWidth, y + boxH, 8f, 8f,
            Paint().apply { color = bgColor; style = Paint.Style.FILL },
        )

        canvas.drawRoundRect(
            margin, y, margin + 4f, y + boxH, 8f, 8f,
            Paint().apply { color = lineColor; style = Paint.Style.FILL },
        )
        canvas.drawRect(
            margin + 2f, y, margin + 4f, y + boxH,
            Paint().apply { color = lineColor; style = Paint.Style.FILL },
        )
    ...
    private fun drawDailySalesChart(
        ...
        canvas.drawRoundRect(
            margin, y, pageWidth - margin, y + chartH + 20f, 8f, 8f,
        ...
        for (s in 0..gridSteps) {
            ...
            canvas.drawLine(
                margin + 4f, gy, pageWidth - margin - 4f, gy,
        ...
        dailySales.forEachIndexed { idx, day ->
            ...
            canvas.drawRoundRect(
                bx, topY, bx + barW, y + chartH, 3f, 3f,
                Paint().apply { color = colorAccent; style = Paint.Style.FILL },
            )

            canvas.drawRoundRect(
                bx, topY, bx + barW, topY + 6f, 3f, 3f,
                Paint().apply { color = colorWhite; style = Paint.Style.FILL; alpha = 40 },
            )

            if (idx % 3 == 0) {
                canvas.drawText(
                    "${day.dayOfMonth}", bx + barW / 2 - 4f,
        ...
        canvas.drawLine(
            margin, y + chartH, pageWidth - margin, y + chartH,
    ...
    private fun drawTopProducts(
        ...
        val headerH = 28f
        canvas.drawRoundRect(
            margin, y, pageWidth - margin, y + headerH, 6f, 6f,
            Paint().apply { color = colorBrand; style = Paint.Style.FILL },
        )

        canvas.drawRect(
            margin, y + headerH / 2, pageWidth - margin, y + headerH,
            Paint().apply { color = colorBrand; style = Paint.Style.FILL },
        )
        ...
        products.take(10).forEachIndexed { idx, product ->
            ...
            canvas.drawRect(
                margin, y, pageWidth - margin, y + rowH,
                Paint().apply {
            ...
            canvas.drawCircle(
                margin + 22f, y + rowH / 2, 12f,
                Paint().apply { color = badgeColor; style = Paint.Style.FILL },
            )
            canvas.drawText(
                "${idx + 1}", margin + 22f, y + rowH / 2 + 4f,
    ...
    private fun drawCategorySales(
        ...
        val barColors = listOf(
            colorAccent, colorSuccess, colorWarning,
    ...
    private fun drawFooter(
        canvas: Canvas, year: Int, month: Int, pageNum: Int, storeName: String,
    ) {
        val footerY = pageHeight - 28f

        canvas.drawRect(
            0f, footerY - 2f, pageWidth.toFloat(), pageHeight.toFloat(),
            Paint().apply { color = colorBrand; style = Paint.Style.FILL },
        )

        canvas.drawText(
            "${storeName.uppercase()} © $year  ·  Powered by ZYPOS  ·  ${getMonthName(month)} $year",
            margin, footerY + 14f,
            regularPaint(8.5f, Color.parseColor("#90CAF9")),
        )

        canvas.drawText(
            "Page $pageNum", pageWidth - margin, footerY + 14f,
    ...
    private fun drawSectionLabel(canvas: Canvas, y: Float, text: String) {
        ...
        canvas.drawLine(
            margin, y + 18f, margin + lp.measureText(text), y + 18f,
